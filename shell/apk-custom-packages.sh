...
# =============== 以下为按需求启用的配置 (25.12.x x86-64 软路由, apk 包格式) ===============
...
# ---------- 系统美化 (Aurora 主题, 与 build25.sh 默认 Argon 不冲突, 开机后 LuCI 里切) ----------
...
# ---------- 科学上网 (硬性: OpenClash + PassWall2 同编译, 实际只启用其一; 25.12 走 apk 下载) ----------
# OpenClash (含内核与依赖, 严格按原版写法; build25.sh 会自动下载 clash_meta apk + GeoIP)
...
# ---------- 首页与网络向导 (quickstart, 25.12 依赖 iStore, 记得 UI 勾 iStore) ----------
...
# filemanager: 文件管理器 (build25.sh 已默认装中文, 这里是主程序)
